...
TElement DynamicVector::getOne( const int &position ) const{
	return this->elems[position];
}

void DynamicVector::update( const int &position, const std::string &new_genre, const int &new_year, const std::string &new_link ) {
	this->elems[position].setGenre(new_genre);
	this->elems[position].setYear(new_year);
	this->elems[position].setLink(new_link);
}

void DynamicVector::updateLike( const int &position, const int &new_year ) {
